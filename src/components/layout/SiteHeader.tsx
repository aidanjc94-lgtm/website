...
import { getPageByKey, headerNavigation } from '@/lib/pageVisibility';

export function SiteHeader() {
  const contactPage = getPageByKey('contact');
  const desktopNavigation = headerNavigation.filter((item) => item.pageKey !== contactPage.pageKey).slice(0, 9);
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-fenland-purple/10 bg-white/95 backdrop-blur">
      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8" aria-label="Main navigation">
        ...
        <div className="hidden lg:block"><CTAButton href={contactPage.href} variant="secondary">{contactPage.navigationLabel}</CTAButton></div>
